...
find.jets.chebpoly.2d <- function(matrix.u, matrix.v, matrix.z, axis.x, axis.y, n.order) {
...
find.jets.dijkstra.2d <- function(u, v, lon, lat, season) {
  ...
  PFJ.lat <- PFJ$SP.J.lat; PFJ.u <- PFJ$SP.J.u; PFJ.v <- PFJ$SP.J.v; PFJ.z <- PFJ$SP.J.z;
  STJ.lat <- STJ$SP.J.lat; STJ.u <- STJ$SP.J.u; STJ.v <- STJ$SP.J.v; STJ.z <- STJ$SP.J.z;
  
  ## Überprüfen, ob Jets weiter als 10 Grad auseinanderliegen.
  ## Falls nicht, Annahme für Single-Jetstream
  SJS.ind <- which(sqrt((PFJ.lat - STJ.lat)**2) < 10)
  
  SJS.lat <- rep(NA, nrow(u)); 
  SJS.u <- SJS.lat; SJS.v <- SJS.lat; SJS.z <- SJS.lat
  ...
  if (length(SJS.ind) != 0) {
    ...
    ## Setze korrespondierende PFJ.* und STJ.* gleich NA
    PFJ.lat[SJS.ind] <- NA;  PFJ.u[SJS.ind]   <- NA;  PFJ.v[SJS.ind]   <- NA;  
    STJ.lat[SJS.ind] <- NA;  STJ.u[SJS.ind]   <- NA;  STJ.v[SJS.ind]   <- NA;  
  }
