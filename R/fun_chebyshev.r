####################################################################################################
## source('~/Master_Thesis/pckg.cheb/R/fun_chebyshev.r')
##
##
...
fkt.cheb.1st <- function(x.axis, n){
  ...
  ## ##
  x.cheb <- if (max(x.axis) - min(x.axis) > 2) pckg.cheb:::fkt.cheb.scale(x.axis) else x.axis  ### ###
  m <- n + 1
...
fkt.cheb.2nd <- function(x.axis, n){
  ...
  ## ##
  x.cheb <- if (max(x.axis) - min(x.axis) > 2) pckg:::fkt.cheb.scale(x.axis) else x.axis
  m <- n + 1
...
fkt.cheb.val <- function(x.axis, cheb.coeff) {
  ...
  cheb.t <- pckg.cheb:::fkt.cheb.1st(x.axis, n)
...
fkt.cheb.deriv <- function(x.axis, cheb.coeff) {
  ...
  if (length(x.axis) != 0) { ### Überprüfen, ob nötig
    n <- length(cheb.coeff) - 1
    m <- n + 1
    cheb.u <- pckg.cheb:::fkt.cheb.2nd(x.axis, n)
...
fkt.cheb.fit <- function(d, x.axis, n){
  x.cheb <- pckg.cheb:::fkt.cheb.scale(x.axis)
  cheb.t <- pckg.cheb:::fkt.cheb.1st(x.axis, n)
  cheb.u <- pckg.cheb:::fkt.cheb.2nd(x.axis, n)
  m <- n + 1
  ## modell berechnungen
  ...
  x.extr <- pckg.cheb:::fkt.cheb.rescale(extr, x.axis = x.axis)
  y.extr <- if (length(extr) != 0) pckg.cheb:::fkt.cheb.val(x.axis = extr, cheb.coeff = cheb.coeff)
