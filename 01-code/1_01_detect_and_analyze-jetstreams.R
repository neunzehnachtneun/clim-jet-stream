...
geopotential <- 250
bound_lower_chebyshev <- 10
bound_upper_chebyshev <- 80
order_polynomial_fit <- 19
...
time_begin <- seq(1, 720, 60)
time_end <- time_begin + 59
for (i in seq(length(time_begin))) {
...
# save.image("stp-b.RData"); load("stp-b.RData")
  if (i == 1) {
    write.table(x = df_jets_month, file = "df_jets_monthly_250hpa.csv", quote = FALSE)
  } else {
  write.table(x = df_jets_month, file = "df_jets_monthly_250hpa.csv", quote = FALSE, 
